...
// ---------------------------------------------------------------------------
// Pomodoro menu bar item
//
// main.js holds no timer logic of its own — the renderer pushes its current
// display state here after every repaint (see app.js's renderPomodoroControl),
// and every click below just relays the same action the header button already
// performs. This keeps @lectio/core's session logic the single source of
// truth instead of growing a second implementation in the main process.
// ---------------------------------------------------------------------------
...
function buildTrayIcon() {
  ...
  // icon, and this affects only the Tray; the dock icon, window icon, and
  // packaged app icon are untouched. Generated by assets/generate-tray-icon.js
  // (same Chromium-canvas pattern as generate-icon.js), it draws the exact
  // same clock path the header's study-timer button already uses, so the two
  // read as the same symbol. Electron auto-loads the @2x sibling for Retina
  // when given the 1x path. setTemplateImage marks it so macOS re-tints the
  // black-on-transparent glyph for light/dark menu bars and the selection
  // highlight — the earlier resized-logo version couldn't do this, since a
  // full-color image isn't a valid template image.
  const img = nativeImage.createFromPath(path.join(__dirname, 'assets', 'pomodoro-tray-icon.png'));
...
function buildTrayMenu() {
  const idle = trayState.phase === 'idle';
    ...
    return Menu.buildFromTemplate([
      {
        ...
        click: () => {
          showMainWindow();
          sendToRenderer('tray-open-pomodoro-modal');
        },
      },
      { type: 'separator' },
...
function refreshTray() {
  ...
  const idle = trayState.phase === 'idle';
  tray.setToolTip(idle ? 'Lectio — Study timer' : `Lectio — ${trayState.label}: ${trayState.clock}`);

  // setTitle (text beside the icon) is macOS/Linux-only in Electron; Windows
  // trays have no equivalent, so this simply does nothing there.
  if (process.platform === 'darwin') {
    tray.setTitle(idle ? '' : ` ${trayState.clock}`);
  }
}
...
ipcMain.on('pomodoro-tray-report', (event, payload) => {
