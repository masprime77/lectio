...
- Added a unified "+" add-sheet (`app/add.tsx`, modal at `/add?context=semester|course|tags`) for the container objects, with all three tabs — Semester · Course · Tags — selectable from every screen; `context` only picks the initial tab (Semesters FAB → Semester, Courses FAB → Course, Course-detail FAB → Tags). The Course and Tags tabs carry a semester picker (pill row from `storage.list()`) that defaults to the semester the sheet was opened from, else the first one, with a "create a semester first" hint when none exist. The Tags tab opens the existing tag editor for the chosen semester via `router.replace` (so closing it returns to the originating screen, not a leftover sheet). The "+" never creates individual readings/tasks — those keep being added from the per-section "+ Add" controls next to the Readings/Tasks headers.
- Header consistency on mobile: the Semesters screen now shows Profile top-left as a drawn person glyph (two accent-colored shapes — no emoji, no icon library) and Edit/Done top-right with the batch Delete (n) control, matching the Courses and Course-detail screens where Edit already sits top-right. The compact "Tags" header button on the Courses screen is kept as the quick entry to the full tag editor.
