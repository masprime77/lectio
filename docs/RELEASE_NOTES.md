...
- Restyled the "+ New" header action as a filled accent-colored button, and replaced the "Sign out" header button with a "Profile" entry opening a new profile screen (`app/profile.tsx`) that shows the signed-in email and the sign-out option (only that, for now).
